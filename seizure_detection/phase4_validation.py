...
Pipeline role:
- receives validated Empatica sessions and onset-derived seizure intervals,
- builds patient-held-out train/validation/test tensors,
- trains Standard and Pro sensor-tier variants,
- chooses thresholds and score-fusion weights on validation data only,
- reports final metrics on the held-out test split,
- exports smoke-tested TensorFlow Lite artifacts for edge-feasibility analysis.
"""
...
# Product-tier map used by the Standard/Pro study. ACC is always in the primary
# branch. Secondary channels are masked to represent practical reduced-sensor
# and add-on configurations without rebuilding the raw dataset for every variant.
...
def split_patient_paths(
    ...
    # The split is made at patient level to avoid leakage from patient-specific
    # physiology, sensor placement, or session artifacts into held-out results.
    ...
    def split_group(patient_ids: List[str]):
        # Seizure and non-seizure patient groups are split separately so each
        # final split keeps a meaningful mix where the available data permits it.
        if not patient_ids:
            return [], [], []
...
def print_label_counts(data: Dict[str, np.ndarray]) -> None:
    # Keep the label summary compact so split checks are easy to read.
...
def split_events(
    ...
    # Session-level splitting is the line that protects us from leakage.
    try:
        from phase2_data_generator_fast import get_actual_session_intervals
    except ImportError:
        from .phase2_data_generator_fast import get_actual_session_intervals
...
def event_level_metrics_from_probabilities(
    ...
    # Convert window probabilities into event-level counts and false-alarm rates.
...
def mask_secondary_data(
    ...
    masked = dict(data)
    # Primary ACC features are unchanged. Only the secondary branch is masked,
    # which makes Standard/Pro comparisons share the same windows and labels.
    for split in ("train", "val", "test"):
...
def parse_feature_env(name: str, default: List[str]) -> Tuple[str, ...]:
    # Parse feature lists from the environment without failing on empty input.
...
def validation_allocation_metrics(
    ...
    # Thresholds and Pro fusion are selected on validation data only. The held-out
    # test split is untouched until the final selected operating point is known.
...
def allocation_selection_score(
    ...
    # Selection favors the dissertation product claim in a bounded way:
    # Pro should not lose events, should improve recall/balanced accuracy when
    # possible, and should reduce false alarms before secondary tie-breakers.
...
def blend_pro_scores(
    ...
    # Pro is modeled as a cumulative tier: the add-on score can boost or temper
    # the Standard detector rather than replacing it outright.
    return (
...
def select_pro_fusion(
    ...
    for weight in pro_fusion_weights():
        # Each candidate blend receives its own validation threshold under the
        # same alarm-budget rule, then is ranked without using test labels.
...
def export_tflite(
    ...
    if not _tflite_smoke_invoke(save_path):
        smoke_policy = os.environ.get("EDGE_FAIL_ON_TFLITE_SMOKE_FAILURE", "0")
        message = f"TFLite smoke inference failed: {save_path}"
        if smoke_policy == "1":
            raise RuntimeError(message)
        print(f"Warning: {message}; keeping exported model and continuing.")
