...
Edge Feasibility
--------------------
This module contains code related to the edge feasibility component of the
seizure detection project. The functions/classes defined herein are used by the
main detection pipeline and related utilities.
"""
...
def load_detection_results() -> Dict:
    """Load the latest detection results artifact."""
...
def load_model(variant_name: str) -> keras.Model:
    """Load a trained Keras checkpoint for the requested variant."""
...
def export_tflite(model: keras.Model, path: Path, mode: str) -> Path:
    """Export a model to TFLite using the requested conversion mode."""
...
def dtype_size(dtype) -> int:
    """Return the byte width of a NumPy dtype."""
    return int(np.dtype(dtype).itemsize)


def tensor_bytes(details: List[Dict]) -> int:
    """Estimate total bytes for allocated TFLite tensors."""
    total = 0
    for item in details:
...
def make_input(detail: Dict, rng: np.random.Generator) -> np.ndarray:
    """Construct a dummy input tensor for a TFLite interpreter."""
...
def benchmark_tflite(path: Path, runs: int = 200, warmup: int = 20) -> Dict:
    """Run a simple local latency benchmark for a TFLite model."""
...
def benchmark_tflite_subprocess(path: Path, runs: int = 200, warmup: int = 20) -> Dict:
    """Run the local TFLite benchmark in a subprocess for isolation."""
...
def inspect_tflite(
    ...
    """Inspect a TFLite flatbuffer and optionally benchmark it."""
    interpreter = tf.lite.Interpreter(
        model_path=str(path),
        num_threads=1,
        experimental_op_resolver_type=(
...
def keras_summary(model: keras.Model, path: Path) -> Dict:
    """Summarize a Keras model artifact."""
    return {
        "path": str(path),
        "size_bytes": int(path.stat().st_size),
        "size_kb": float(path.stat().st_size / 1024.0),
...
def deployment_notes(tflite_payload: Dict) -> List[str]:
    """Generate human-readable deployment notes from a TFLite profile."""
    notes = []
...
def write_markdown_summary(report: Dict) -> None:
    """Write a short Markdown summary of the edge-feasibility report."""
    calibrated_dtypes = []
...
def main() -> None:
    """Run the edge-feasibility analysis for the selected models."""
...
def benchmark_one_cli() -> int:
    """CLI entry point for benchmarking one TFLite model."""
    import argparse
