...
This script defines several baseline neural network models (LSTM, CNN‑LSTM, Transformer, Autoencoder) and provides utilities for training, evaluating, and reporting their performance on seizure detection data.

The pipelines mirror the main TSMixer detector to allow fair comparison without altering the primary detection flow.
"""
...
def set_seed() -> int:
    """Set random seed for reproducibility and return it.

    The seed is read from the ``DETECTION_RANDOM_SEED`` environment variable
    or defaults to ``42``. It seeds both NumPy and TensorFlow RNGs.
    """
...
def combine_inputs(primary: np.ndarray, secondary: np.ndarray) -> np.ndarray:
    """Concatenate primary and secondary sensor inputs along the last axis.

    Parameters
    ----------
    primary: np.ndarray
        Primary sensor window data.
    secondary: np.ndarray
        Secondary sensor window data.

    Returns
    -------
    np.ndarray
        Combined input ready for model consumption.
    """
...
def compile_supervised_model(model: keras.Model) -> keras.Model:
    """Compile a Keras model for supervised binary classification.

    The model is compiled with Adam optimizer, binary cross‑entropy loss, and
    a set of metrics useful for seizure detection (AUC, PR‑AUC, precision,
    recall).
    """
    model.compile(
...
def build_lstm(input_shape: Tuple[int, int]) -> keras.Model:
    """Create a simple LSTM baseline model.

    Parameters
    ----------
    input_shape: Tuple[int, int]
        Shape of the input sensor window (timesteps, features).
...
def build_cnn_lstm(input_shape: Tuple[int, int]) -> keras.Model:
    """Create a CNN‑LSTM baseline model.

    This architecture first extracts local patterns with a 1‑D convolution
    before feeding the features to an LSTM layer.
    """
    inputs = layers.Input(shape=input_shape, name="sensor_window")
...
def build_transformer(input_shape: Tuple[int, int]) -> keras.Model:
    """Create a compact Transformer baseline model.

    Uses a simple feed‑forward network followed by multi‑head attention and
    layer normalisation.
    """
    inputs = layers.Input(shape=input_shape, name="sensor_window")
...
def build_autoencoder(input_shape: Tuple[int, int]) -> keras.Model:
    """Create a sequence autoencoder for unsupervised anomaly scoring.

    The autoencoder reconstructs the input window; reconstruction error is
    later used as an anomaly score.
    """
    inputs = layers.Input(shape=input_shape, name="sensor_window")
...
def train_supervised(
    ...
    """Train a supervised baseline model with early stopping.

    Parameters
    ----------
    name: str
        Identifier for the baseline (e.g., ``lstm``).
    model: keras.Model
...
def train_autoencoder(
    ...
    """Train the autoencoder baseline with early stopping.
    """
