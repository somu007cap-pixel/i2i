...
def load_json(path: Path) -> dict[str, Any] | None:
    """Load JSON from disk and tolerate missing files."""
    if not path.exists():
        return None
    try:
...
def add(checks: list[dict[str, Any]], name: str, status: str, detail: str) -> None:
    """Append one audit check entry."""
    checks.append({"name": name, "status": status, "detail": detail})


def build_scientific_checklist(results: dict[str, Any]) -> dict[str, str]:
    """Return a few core scientific-rigor checks for the detection report."""
...
def audit_detection(checks: list[dict[str, Any]]) -> None:
    """Populate audit entries for the detection results artifact."""
...
def audit_edge(checks: list[dict[str, Any]]) -> None:
    """Populate audit entries for the edge-feasibility artifact."""
...
def extract_pdf_text(path: Path) -> str:
    """Extract text from a PDF if the parser is available."""
    try:
...
def extract_docx_text(path: Path) -> str:
    """Extract text from a DOCX by using python-docx or raw XML fallback."""
    if not path.exists():
        return ""
    try:
...
def audit_report(checks: list[dict[str, Any]]) -> None:
    """Inspect the submitted report for scope and wording regressions."""
...
def write_outputs(checks: list[dict[str, Any]]) -> None:
    """Write the audit JSON and markdown summary to disk."""
...
def main() -> None:
    """Run the readiness audit and write the results to disk."""
