...
def event_text(metrics: dict[str, Any]) -> str:
    """Format event-level detected/total counts."""
...
def alert_text(metrics: dict[str, Any]) -> str:
    """Format alert-level detected/total counts."""
...
def alert_false_alarm_text(metrics: dict[str, Any]) -> str:
    """Format alert-level false alarm rate."""
    event = metrics.get("event_alert_level") or {}
    if not event:
        return "n/a"
...
def event_sensitivity_ci(metrics: dict[str, Any]) -> str:
    """Format event sensitivity with confidence interval when available."""
    event = metrics.get("event_level") or {}
...
def row_from_results(folder: Path) -> dict[str, Any] | None:
    """Build a study row from an archived experiment folder."""
...
def collect_experiments() -> list[dict[str, Any]]:
    """Collect all archived TSMixer experiment rows."""
...
def metric(row: dict[str, Any], tier: str, key: str) -> float:
    """Read one numeric metric from a Standard or Pro row."""
    try:
...
def summarize_group(rows: list[dict[str, Any]]) -> dict[str, Any]:
    """Aggregate repeated-run metrics into mean/std summaries."""
...
def robustness_groups(rows: list[dict[str, Any]]) -> dict[str, list[dict[str, Any]]]:
    """Group archived runs by robustness-study family."""
...
def best_baseline() -> dict[str, Any] | None:
    """Select the strongest baseline row by event and ranking metrics."""
...
def baseline_operating_point_rows() -> list[str]:
    """Build markdown rows for baseline false-alarm operating points."""
    report = load_json(BASELINE_REPORT)
    if not report:
        return []
    rows = [
...
def failure_notes(row: dict[str, Any]) -> list[str]:
    """Summarize missed events by tier for failure analysis."""
    notes = []
...
def write_report() -> None:
    """Write the final study report from archived artifacts."""
...
def mean_std_text(summary: dict[str, Any], digits: int = 4) -> str:
    """Format a mean/std pair for report tables."""
