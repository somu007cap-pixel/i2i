...
FINAL_STUDY_PATH = OUTPUT_DIR / "final_study_report.json"
...
def load_json(path: Path) -> dict[str, Any]:
    """Helper used by the reporting pipeline."""
    if not path.exists():
        return {}
...
def plot_ablation_summary() -> Path | None:
    """Helper used by the reporting pipeline."""
    final_report = load_json(FINAL_STUDY_PATH)
    groups = final_report.get("robustness_groups") or {}
    if not groups:
    ...
    label_map = {
        "seed_variability::booster_tuned_patch8_base_eda_boost_ppg_temp": "Seed: base EDA,\nboost PPG+TEMP",
        "label_policy::booster_tuned_patch8_base_eda_boost_ppg_temp": "Label policy: base EDA,\nboost PPG+TEMP",
        "seed_variability::patch8_base_temp_boost_ppg_eda": "Seed: base TEMP,\nboost PPG+EDA",
        "label_policy::patch8_base_temp_boost_ppg_eda": "Label policy: base TEMP,\nboost PPG+EDA",
    }
    ordered_keys = [key for key in label_map if key in groups]
    if not ordered_keys:
        return None

    names = [label_map[key] for key in ordered_keys]
    pro_event = [groups[key]["pro_event_sensitivity"]["mean"] for key in ordered_keys]
    pro_event_std = [groups[key]["pro_event_sensitivity"]["std"] for key in ordered_keys]
    pro_auc_gain = [groups[key]["pro_auc_gain"]["mean"] for key in ordered_keys]
    pro_auc_gain_std = [groups[key]["pro_auc_gain"]["std"] for key in ordered_keys]

    fig, axes = plt.subplots(1, 2, figsize=(12.5, 4.6), sharey=True)
    y = np.arange(len(ordered_keys))
    axes[0].barh(y, pro_event, xerr=pro_event_std, color=COLORS["success"], alpha=0.9)
    axes[0].set_yticks(y)
    axes[0].set_yticklabels(names)
    axes[0].invert_yaxis()
    axes[0].set_xlim(0.0, 0.85)
    axes[0].set_title("Pro Event Sensitivity")
    axes[0].set_xlabel("Detected-event fraction")
    axes[1].barh(y, pro_auc_gain, xerr=pro_auc_gain_std, color=COLORS["pro"], alpha=0.9)
    axes[1].axvline(0, color="#444444", linewidth=1)
    axes[1].set_title("Pro AUC Gain Over Standard")
    axes[1].set_xlabel("ROC-AUC gain")
    fig.suptitle("Robustness and Sensor-Tier Ablation Summary")
    fig.tight_layout()
    return save(fig, "06_ablation_summary.png")
...
def plot_edge_footprint(edge_report: dict[str, Any]) -> Path | None:
    ...
    for tier in ("standard", "pro"):
        ...
        latency_proxy = tflite.get("latency_proxy") or {}
        ...
        latency.append(float(latency_proxy.get("latency_ms_p50", latency_proxy.get("latency_ms_mean", 0.0))))
    ...
    for ax, values, title, ylabel in [
        ...
        bars = ax.bar(tiers, values, color=[COLORS["standard"], COLORS["pro"]])
        ax.set_title(title)
        ...
        for bar, value in zip(bars, values):
            ax.text(
                ...
                bar.get_height(),
                f"{value:.1f}" if ylabel == "KB" else f"{value:.2f}",
                ha="center",
                va="bottom",
                fontsize=9,
            )
