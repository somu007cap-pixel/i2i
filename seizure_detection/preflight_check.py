...
def check(condition: bool, name: str, detail: str, failures: list[dict]) -> None:
    """Record a named preflight result."""
...
def command_version(command: list[str], timeout: int = 20) -> str | None:
    """Return the first line of a tool's version output when available.
    Allows specifying a custom timeout (seconds) for the subprocess call.
    """
...
def find_generated_artifacts() -> list[str]:
    """List generated artifacts outside the current marathon output tree."""
    found: list[str] = []
...
def data_inventory() -> dict:
    """Summarize patient folders, sessions, and label files."""
...
def find_patient_label_file(patient_dir: Path) -> Path | None:
    """Find the patient seizure timestamp file for a Mayo folder."""
...
def main() -> int:
    """Run all preflight checks and write a JSON report."""
    failures: list[dict] = []
    report: dict = {
    ...
    cli_versions = {
        ...
        "edge_impulse": command_version(["edge-impulse-daemon", "--version"], timeout=60),
    }
