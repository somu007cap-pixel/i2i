...
def load_json(path: Path):
    """Load one JSON artifact if it exists."""
    if not path.exists():
        return None
...
def selected_model_edge(edge):
    """Return compact edge metrics for the selected Pro model."""
    if not edge:
        return {}
...
def row_for_experiment(folder: Path):
    """Build one summary row from an archived experiment folder."""
...
def score(row):
    """Rank experiments by Pro quality metrics."""
    return (
...
def folder_run_date(folder_name: str) -> str:
    """Extract the run date prefix from an archived experiment folder name."""
...
def folder_timestamp(folder_name: str):
    """Parse the timestamp prefix from an archived experiment folder name."""
    parts = folder_name.split("_", 2)
    if len(parts) < 2:
        return None
    try:
...
def latest_marathon_tsmixer_window():
    """Return the latest matrix-run start and end timestamps from status files."""
    status_files = sorted(
...
def product_score(row):
    """Rank experiments by product-tier usefulness."""
...
def parse_event_count(value: str) -> int:
    """Extract detected-event count from a 'detected/total' metric string."""
    try:
...
def latest_rows(rows):
    """Select rows from the latest matrix run when possible."""
...
def append_rows(lines, rows):
    """Append a markdown table for experiment rows."""
    lines += [
...
def main():
    """Build the TSMixer experiment summary markdown file."""
    rows = []
