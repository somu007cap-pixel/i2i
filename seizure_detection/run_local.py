...
def has_seizure_session(
    ...
    # Keep this check narrow: a session counts only if it overlaps a seizure interval.
    try:
...
def get_patient_id(session_path: str) -> str:
    # Patient folders are the stable split key for held-out validation.
    return os.path.basename(os.path.dirname(os.path.abspath(session_path))) or "unknown"
...
def summarize_sessions(
    ...
    # Summarize by patient and by unique seizure episode so the split audit stays honest.
    try:
        from phase2_data_generator_fast import get_actual_session_intervals
    except ImportError:
        from .phase2_data_generator_fast import get_actual_session_intervals
...
def run_pipeline():
    # This is the real local prediction entrypoint used by the campaign wrapper.
    print("\n" + "=" * 60)
    ...
    base_path = os.environ.get("PREDICTION_BASE_PATH", os.path.dirname(SCRIPT_DIR))
    output_dir = os.environ.get(
        "PREDICTION_OUTPUT_DIR", os.path.join(base_path, "prediction_outputs_local")
    )
    patient_prefix = os.environ.get("PREDICTION_PATIENT_PREFIX", "Mayo_")
    ...
    session_paths = get_session_paths(base_path, patient_prefix=patient_prefix)
