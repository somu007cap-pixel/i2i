...
def _experiment_template(name: str) -> Dict:
    """Return the configured experiment template by name."""
...
def _append_robustness_experiments() -> List[str]:
    """Add seed and label-duration robustness variants to the experiment list."""
    names: List[str] = []
...
def run_command(command: List[str], env: Dict[str, str], log_path: Path) -> int:
    """Run one experiment command and stream logs to disk."""
...
def archive_file(source: Path, target_dir: Path, name: str) -> None:
    """Copy an artifact into the archive directory if it exists."""
    if source.exists():
        shutil.copy2(source, target_dir / name)


def archive_outputs(experiment: Dict, target_dir: Path) -> None:
    """Archive the latest detection artifacts for one experiment."""
...
def update_metadata(target_dir: Path, **updates) -> None:
    """Update the persisted experiment metadata file."""
...
def build_env(experiment: Dict, args) -> Dict[str, str]:
    """Build the environment for one experiment run."""
    env = os.environ.copy()
    env.update(
        {
            "PYTHONUTF8": "1",
...
def run_experiment(experiment: Dict, args) -> int:
    """Run a single experiment, archive artifacts, and record status."""
...
def main() -> int:
    """Entry point for the staged TSMixer experiment matrix."""
    ...
    parser.add_argument(
        "--resume",
        action="store_true",
        help="Deprecated and rejected: every selected experiment must execute.",
    )
    ...
    if args.resume:
        print("ERROR: --resume is disabled. The zero-skip policy requires every selected experiment to execute.")
        return 2

    EXPERIMENT_DIR.mkdir(parents=True, exist_ok=True)
    if not selected:
        print("ERROR: zero experiments selected; refusing a successful no-op run.")
        return 3
    executed = 0
    failures = 0
    ...
    if executed == 0:
        print("ERROR: no experiment executed; refusing success.")
        return 3
